...
class SpotifyAccount:
    ...
    def __init__(
        ...
        self._spotify = Spotify(auth=self.sessions["external"].token["access_token"])

        LOGGER.debug(
            "Account `%s` TOKEN", self.sessions["external"].token["access_token"]
        )

        self._internal_cont = Spotify(auth=self.sessions["internal"].token)
    ...
    async def async_ensure_tokens_valid(
        ...
        for key, session in self.sessions.items():
            ...
            if key == "external":
                ...
                LOGGER.debug(
                    "Refreshing api tokens for Spotify Account: %s",
                    token["access_token"],
                )
            if key == "internal":
                self._internal_cont.set_auth(token)
                LOGGER.debug("Refreshing api tokens for Spotify Account: %s", token)
    ...
    async def async_devices(self, force: bool = False) -> list[dict]:
        ...
        if force or dataset.is_expired:
            ...
            async with dataset.lock:
                data = await self.hass.async_add_executor_job(self._spotify.devices)
    ...
    async def async_get_track(self, uri: str) -> dict:
        ...
        result = await self.hass.async_add_executor_job(
            self._spotify.track, uri, self.country
        )
    ...
    async def async_playlists_count(self) -> int:
        ...
        return await self._async_get_count(self._spotify.current_user_playlists)
    ...
    async def async_search(self, query: SearchQuery, max_items: int = 20) -> list[dict]:
